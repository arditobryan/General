...
#tf_dataset_extractor v1.1
class tf_dataset_extractor:
  ...
  def cross_sectional_split(self, test_size): #Per cross-sectional data
    X = pd.DataFrame()
  ...
  class Extract:
    ...
    def categorical(self, X_categorical_1, label_encoder=False): #mentre per numerical è default, meglio mettere opzione automatica per categorical
      global X_categorical
      global X
  ...
  class Partition:
    ...
    def label_encoder(self, partitions, dict1=None, to_float=False): #se viene passato un dataframe a + colonne ok, altrimenti err
      #che sia una Series o un Datafram non ci sono problemi, la corregge
      global X
      #prima si opera su X, poi si staccano y, categorical, numerical
    ...
    def normalize(self, partitions): #se viene passato un dataframe a + colonne ok, altrimenti err, DEL?
      #prima si opera su X, poi si staccano y, categorical, numerical
      def algo(col):
        global X
        x = X[col].values #returns a numpy array
        X.pop(col) #si duplica altrimenti
        min_max_scaler = preprocessing.MinMaxScaler()
        x_scaled = min_max_scaler.fit_transform(x.reshape(-1, 1)) #x_scaled = min_max_scaler.fit_transform(x): se non utilizziamo reshape da errore
        x_scaled = pd.DataFrame(x_scaled)
        #le ridiamo il nome
        x_scaled.rename(columns={0:col}, inplace=True)
        #lo riattacchaimo
